...
namespace PharmacyAssistant
{
    ...
    {
        private delegate void TreeviewUpdater(string Key, string Text, int ImageIndex, int SelectedImageIndex);
        ...
        private void AddNodeToTreeview(string Key, string Text, int ImageIndex, int SelectedImageIndex)
        {
            if (tvwFolders.InvokeRequired)
            {
                this.Invoke(UpdateDelegate, Key, Text, ImageIndex, SelectedImageIndex);
            }
            else
            {
                //TreeNode RootNode = tvwFolders.Nodes.Add("Documents", "Documents", 0, 0);
                TreeNode RootNode = tvwFolders.Nodes.Add(Key, Text, ImageIndex, SelectedImageIndex);

                GetFTPFileList(RootNode, "/documents",false, PharmacyAssistant.Global.FTPEntrySelection.Folder);
        ...
        {
            MessageBox.Show("Delete Folder feature not implemented at this time");
        }
        ...
        private void GetFTPFileList(TreeNode ParentNode, string Path, bool Recursive, PharmacyAssistant.Global.FTPEntrySelection Selection)
        {
            ...
            List<FTPEntry> Entries = Global.GetFTPDirectoryEntries(true, Path, Selection);
        ...
        private void RefreshFileList(string SearchTerm)
        {
            if (tvwFolders.SelectedNode != null)
            {
                ...
                FTPEntry Entry = (FTPEntry)Node.Tag;
                ...
                if (Entry != null && Entry.IsFolder)
                {
                    ...
                    foreach (FTPEntry FileEntry in Entries)
                    {
                        ...
                        // Get corresponding Document information
                        var SelectedDocument = (from Document d in _Documents
                                                where (d.FileName == FileEntry.Filename && d.Path == FileEntry.Path)
                                                select d).FirstOrDefault();  // was .SingleOrDefault()

                        if (SearchTerm != "" && SelectedDocument != null)
                        {
                            Console.WriteLine("Looking through keywords: " + SelectedDocument.Keywords);
                        ...
                        if (AddDocument)
                        {
                            ...
                            //Document.SubItems.Add(FileEntry.Path);
        ...
        {
            if (tvwFolders.SelectedNode.Tag != null) _SelectedFTPEntry = (FTPEntry)tvwFolders.SelectedNode.Tag;
        ...
        {
            AddNodeToTreeview("Documents", "Documents", 0, 0);
